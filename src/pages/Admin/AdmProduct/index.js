...
export function ProductAdminpage() {
  ...
  const gotoDetail = (id) => {
    navigate(`/AdmEditProduct/${id}`);
  };
  const [dataProduct, setDataProduct] = useState([]);
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/v1/Products?limit=10&page=1")
      .then((res) => {
      ...
      <main className="container row px-5 mx-5">
        <div className="col-lg-4 border-end mt-5">
          ...
          <div className="btn  text-center d-grid gap-2">
            <button className="btn  text-center d-grid gap-2 bg-warning" >Claim Coupon</button>
          </div>
        </div>
        {/* Akhir Aside  */}
        {/* Menu Product */}
